...
void MipsCpu::inst_mem() {
    switch (ctrlsigs_.ld_type) {
        case LoadType::LD_W: {
            int addr       = alu_out_;
            bool read_conf = (addr & CONF_MASK) == CONF_BASE;
            if (read_conf) {
                load_data_ = conf_ptr_->get_word(addr);
            } else {
                load_data_ = dram_ptr_->get_word(addr);
            }
            break;
        }
    }
    ...
    switch (ctrlsigs_.st_type) {
        case StoreType::ST_W: {
            int addr        = alu_out_;
            bool write_conf = (addr & CONF_MASK) == CONF_BASE;
            if (write_conf) {
                conf_ptr_->set_word(addr, store_data_);
            } else {
                dram_ptr_->set_word(addr, store_data_);
            }
            break;
        }
    }
...
void MipsCpu::inst_wb() {
    ...
    if (ctrlsigs_.rf_wen) {
        ...
        // trace log
        trace_ << std::hex << pc_ << "\t" << std::dec << std::to_string(rf_waddr) << "\t"
               << std::hex << rf_wdata << std::endl;
    }
}