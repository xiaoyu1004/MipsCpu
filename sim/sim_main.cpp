...
bool verify(VTop* top, std::ifstream& fs) {
  ...
  printf(
      ...
      (int)top->debug_wb_pc, (int)top->debug_wb_rf_waddr, (int)top->debug_wb_rf_wdata, trace_wb_pc,
      trace_rf_waddr, trace_rf_wdata);
...
int main(int argc, char** argv) {
  ...
  tfp->open(argc > 3 ? argv[3] : "dump.vcd");  // Open the dump file
#endif
  ...
  std::string tfile("/home/xiaoyu/workspace/MipsCpu/build/trace.txt");
  std::ifstream fs = open_trace(tfile);

  do {
    tick();

    run_finish = top->debug_wb_pc >= (inst_cnt - 4) * 4 - 4;
    if (run_finish) {
      break;
    }
  ...
  std::cout << "Simulation terminated at time " << main_time << " (cycle " << main_time / 10 << ")"
            << std::endl;
  ...
  std::cout << "simulation exit! program run " << (run_finish ? "finish" : "not finish")
